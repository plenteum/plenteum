...
namespace
{
  ...
  const size_t testGrantedFullRewardZone = CryptoNote::parameters::CRYPTONOTE_BLOCK_GRANTED_FULL_REWARD_ZONE;
  ...
  protected:
    ...
    void do_testV1(uint64_t alreadyGeneratedCoins, uint64_t fee) {
      m_blockTooBig = !m_currency.getBlockReward(CryptoNote::BLOCK_MAJOR_VERSION_1, testMedian, testBlockSize, alreadyGeneratedCoins, fee, m_blockReward, m_emissionChange);
    }
